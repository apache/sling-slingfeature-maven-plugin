...
public class SubstitutionTest {
    ...
    public void testReplaceMavenVars() {
        ...
        assertEquals(
                "hellofoogoodbyefoo",
                Substitution.replaceMavenVars(proj, false, false, new String[] {"test"}, "hello${test}goodbye${test}"));
        assertEquals(
                "hello${test}goodbye${test}",
                Substitution.replaceMavenVars(proj, false, false, null, "hello${test}goodbye${test}"));
    }
    ...
    public void testReplaceMavenVarsWithSystemProperties() {
        ...
        try {
            ...
            assertEquals(
                    "hellobargoodbyebar",
                    Substitution.replaceMavenVars(proj, true, false, null, "hello${test}goodbye${test}"));
        } finally {
    ...
    public void testReplaceAdditionalPropertiesMavenVarsWithSystemProperties() {
        ...
        try {
            ...
            assertEquals(
                    "hellobargoodbyefoo2",
                    Substitution.replaceMavenVars(
                            proj, false, false, new String[] {"test", "test2"}, "hello${test}goodbye${test2}"));
        } finally {
            // Restore the system properties
            System.setProperties(storedProps);
        }
