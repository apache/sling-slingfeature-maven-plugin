...
public class JarDecompressorTest {
    ...
    public void testDecompress() throws Exception {
        ...
        File uj = Files.createTempFile(getClass().getSimpleName(), "uncompressed.jar")
                .toFile();
    ...
    public void testSignedJar() throws Exception {
        File cj = new File(
                getClass().getResource("/repository/compressed2-signed.jar").getFile());
        File uj = Files.createTempFile(getClass().getSimpleName(), "still-compressed.jar")
                .toFile();
    ...
    public void testJarWithEmbeddedJar() throws Exception {
        ...
        File uj = Files.createTempFile(getClass().getSimpleName(), "embedded-uncompressed.jar")
                .toFile();
        ...
        try {
            ...
            File tf =
                    new File(getClass().getResource("/repository/textfile.txt").getFile());
            assertEquals(tf.length(), actualJar.get("textfile.txt").length);
            File cf = new File(
                    getClass().getResource("/repository/compressed.jar").getFile());
            assertTrue(
                    "Embedded compressed jar should be decompressed",
            ...
            File sf = new File(
                    getClass().getResource("/repository/compressed2-signed.jar").getFile());
            assertEquals(
                    "Signed jar should not be decompressed",
                    sf.length(),
                    actualJar.get("compressed2-signed.jar").length);
        } finally {
            assertTrue(uj.delete());
        }
    }
