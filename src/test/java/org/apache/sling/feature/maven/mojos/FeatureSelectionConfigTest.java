...
public class FeatureSelectionConfigTest {
    ...
    public void testOrdering() {
        ...
        // check ordering
        assertEquals(
                FeatureSelectionConfig.SelectionType.CLASSIFIER,
                cfg.getSelections().get(0).type);
        assertEquals("classifier", cfg.getSelections().get(0).instruction);

        assertEquals(
                FeatureSelectionConfig.SelectionType.ARTIFACT,
                cfg.getSelections().get(1).type);
        assertEquals("gr:ar:1.1", cfg.getSelections().get(1).instruction);

        assertEquals(
                FeatureSelectionConfig.SelectionType.FILES_INCLUDE,
                cfg.getSelections().get(2).type);
        assertEquals("INC1", cfg.getSelections().get(2).instruction);

        assertEquals(
                FeatureSelectionConfig.SelectionType.CLASSIFIER,
                cfg.getSelections().get(3).type);
        assertEquals("another", cfg.getSelections().get(3).instruction);

        assertEquals(
                FeatureSelectionConfig.SelectionType.FILES_INCLUDE,
                cfg.getSelections().get(4).type);
        assertEquals("INC2", cfg.getSelections().get(4).instruction);

        assertEquals(
                FeatureSelectionConfig.SelectionType.REFS_INCLUDE,
                cfg.getSelections().get(5).type);
